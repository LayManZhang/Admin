...
class Ajax extends Backend
{
    ...
    protected $noNeedLogin = ['lang','addCompany','upload'];
    ...
    //从105000开始  210 500
    public function addCompany($page=1113){
        set_time_limit(0);
        // 1. 初始化
        $url = 'http://opendata.sz.gov.cn/api/1564501785/1/service.xhtml';
        $rows = 1000;
        $params = [
            'appKey' => '7c02d1be0ffe4e24af6ca94f652ce6a4',
            'page' => $page,
            'rows' => $rows,
        ];

        $ch =  Http::sendRequest($url,$params,'GET');
        if($ch['ret']){
            $res = json_decode($ch['msg'],true);
            if(empty($res['data'])){
                return 'success';
            }else{
                foreach ($res['data'] as $val){
                    db('company')->insert($val);
                }
                sleep(5);
                $page++;
                self::addCompany($page);
            }
        }else{
            self::addCompany($page);
        }
