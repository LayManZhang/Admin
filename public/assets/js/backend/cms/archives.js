    ...
    var Controller = {
        ...
        content: function () {
            ...
            $.each(Config.fields, function (i, j) {
                ...
                } else if (j.type == 'radio' || j.type == 'check' || j.type == 'select' || j.type == 'selects' || j.type == 'checkbox' ) {
                    data.formatter = Controller.api.formatter.content;
                    data.extend = j.content;
                } else if (  j.type == 'switch' && j.field == 'Status' ) {
                    data.formatter = Controller.api.formatter.content;
                    data.extend = {1:'开放申报中',0:'申报已结束'};
                } else if ( j.type == 'custom' && j.field == 'xmjb' ) {
                    data.formatter = Controller.api.formatter.content;
                    data.extend = j.content;
                } else if ( j.type == 'custom' && j.field == 'ssxm' ) {
                    data.formatter = Controller.api.formatter.content;
                    data.extend = j.content;
                }
                columns.push(data);
        ...
        edit: function () {
            ...
            Fast.api.ajax({
                ...
                Form.api.bindevent($("#extend"));
                //给三级联动设置值  自定义字段赋值
                var xmjb = Config.row.xmjb;
                xmjb = xmjb.split(',');
                $('.province').attr('data-value',xmjb[0]);
                $('.city').attr('data-value',xmjb[1]);
                $('.area').attr('data-value',xmjb[2]);
                $('#c-ssxm').attr('value',Config.row.ssxm);
                $('#c-Informationization').attr('value',Config.row.Informationization);
                $('#c-Brandput').attr('value',Config.row.Brandput);
                $('#c-Researchput').attr('value',Config.row.Researchput);
                $('#c-SalesProceeds').attr('value',Config.row.SalesProceeds);
                $('#c-Ratal').attr('value',Config.row.Ratal);
                $('#c-Equipmentput').attr('value',Config.row.Equipmentput);
                $('#c-Max_SalesProceeds').attr('value',Config.row.Max_SalesProceeds);
                Form.api.bindevent($('.province'));
                Form.api.bindevent($('.city'));
                Form.api.bindevent($('.area'));
                Form.api.bindevent($('#c-ssxm'));
                Form.api.bindevent($('#c-Informationization'));
                Form.api.bindevent($('#c-Brandput'));
                Form.api.bindevent($('#c-Researchput'));
                Form.api.bindevent($('#c-SalesProceeds'));
                Form.api.bindevent($('#c-Ratal'));
                Form.api.bindevent($('#c-Equipmentput'));
                Form.api.bindevent($('#c-Max_SalesProceeds'));
                return false;
            });
        },
        api: {
