...
class Comment Extends Model
{
    ...
    protected static function init()
    {
        ...
        self::afterInsert(function ($row) use ($config) {
            ...
            //增加积分
            $config['score']['postcomment'] && \app\common\model\User::score($config['score']['postcomment'], $row->user_id, '发布评论');
        });
    ...
    {
        return $this->belongsTo("app\common\model\User")->field('id,username,nickname,avatar,email,bio')->setEagerlyType(1);
    }
    ...
    {
        return $this->belongsTo("app\common\model\User", "reply_user_id", "id", [], "LEFT")->field('id,username,nickname,avatar,email,bio')->setEagerlyType(1);
    }
