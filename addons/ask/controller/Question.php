...
class Question extends Base
{
    ...
    public function index()
    {
        ...
        $categoryId = $this->request->request('category', null);
        $category = null;
        if ($categoryId) {
        ...
        $questionList = \addons\ask\model\Question::getIndexQuestionList($type, $categoryId);
    ...
    public function show()
    {
        ...
        //付费阅读过期且没有解答时退款
        if ($question->is_reward_expired && $question->price > 0 && !$question->answers) {
            $question->refund();
        }
        ...
        $question->setData(['tags' => \addons\ask\model\Tag::getTags('question', $question->id)]);
    ...
    public function post()
    {
        if ($this->request->isPost()) {
            ...
            if (!$this->auth->email) {
                $this->error('暂未绑定邮箱无法进行提问');
            }
            if (!$this->auth->mobile) {
                $this->error('暂未绑定手机号无法进行提问');
            }
    ...
    public function adopt()
    {
        ...
        try {
            ...
            if ($question->price > 0) {
                ...
                //最佳答案分成
                $systemRatio > 0 && User::money($systemRatio * $question->price, $config['system_user_id'], '最佳答案分成');
                $userRatio > 0 && User::money($userRatio * $question->price, $answer->user_id, '最佳答案分成');
            }

            //只有当回答者与提问者不同时才增加积分和消息
            if ($answer->user_id != $question->user_id) {
                //增加积分
                $config['score']['bestanswer'] && User::score($config['score']['bestanswer'], $answer->user_id, '采纳最佳答案');
                //更新动态
    ...
    public function reward()
    {
        ...
        $money = (float)$this->request->request("money");
        if ($money <= 0) {
    ...
    public function delete()
    {
        ...
        if (!Service::isAdmin()) {
            ...
            if ($question->price > 0 && $question->answers > 0) {
    ...
    public function transfer()
    {
        ...
        try {
            ...
            Db::rollback();
            $this->success("转移失败，失败原因：" . $e->getMessage());
        }
